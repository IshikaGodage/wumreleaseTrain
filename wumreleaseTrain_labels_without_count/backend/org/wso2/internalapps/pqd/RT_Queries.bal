...
const string GET_REDMINE_RELEASE_DETAILS = "SELECT kernel_version AS kernalVersion, platform_version AS platformVersion, product_name AS productName, product_version AS productVersion, description AS description, applies_to AS appliesTo, bug_fixes AS bugFixesJson  FROM updates where  date(from_unixtime(floor(timestamp/1000)))=?;";
